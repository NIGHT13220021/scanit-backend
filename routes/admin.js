...
router.get('/stats', authAdmin, async (req, res) => {
  try {
    ...
    const { count: productCount } = await supabase
      .from('store_products').select('id', { count: 'exact' })
      .eq('store_id', store_id).eq('is_available', true);
  ...
  try {
    ...
    const enriched = (sessions || []).map(s => {
      ...
      const total = items.reduce((sum, item) =>
        sum + (item.quantity * (item.store_products?.price || 0)), 0);
...
router.get('/store/qr', authAdmin, async (req, res) => {
  try {
    ...
    const { data: store, error } = await supabase
      .from('stores').select('name, entry_qr_code').eq('id', store_id).single();
...
router.get('/analytics', authAdmin, async (req, res) => {
  try {
    ...
    return res.json({ success: true, revenue_chart: revenueChart, top_products: topProducts });
...
router.get('/superadmin/stats', authSuperAdmin, async (req, res) => {
  try {
    const { count: storeCount } = await supabase
      .from('stores').select('id', { count: 'exact' }).eq('is_active', true);

    const { count: userCount } = await supabase
      .from('users').select('id', { count: 'exact' }).eq('role', 'customer');
...
router.post('/superadmin/stores', authSuperAdmin, async (req, res) => {
  try {
    ...
    const { data: existingUser } = await supabase
      .from('users').select('id').eq('phone', owner_phone).single();
...
router.put('/superadmin/stores/:id/billing', authSuperAdmin, async (req, res) => {
  try {
    ...
    const { error } = await supabase.from('stores').update({ is_active, plan }).eq('id', id);
    if (error) throw error;
